...
// Lightweight client-side utilities for visual updates during interaction

// Map risk level to handle color index (for visual display)
export function riskLevelToHandleColor(risk: RiskLevel): 0 | 1 | 2 {
  switch (risk) {
    case "low": return 0;  // low debt in front = high risk = red
    case "medium": return 1; // medium risk = yellow
    case "high": return 2;   // high debt in front = low risk = green
  }
}
...
export function riskLevelToColor(risk: RiskLevel): string {
  switch (risk) {
    case "low": return RISK_COLORS.high;   // low debt in front = high risk = red
    case "medium": return RISK_COLORS.medium; // medium risk = yellow
    case "high": return RISK_COLORS.low;    // high debt in front = low risk = green
  }
}
...
export function calculateGradientGeometry(riskZones: {
  ...
  return [
    ...
    { x: riskZones.highRiskThreshold * 100, width: (riskZones.mediumRiskThreshold - riskZones.highRiskThreshold) * 100, index: 1 },
    { x: riskZones.mediumRiskThreshold * 100, width: (1 - riskZones.mediumRiskThreshold) * 100, index: 2 },
  ];
}
