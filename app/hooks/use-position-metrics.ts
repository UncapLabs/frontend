...
export function usePositionMetrics({
  ...
  return useMemo(() => {
    ...
    const totalValue =
      collateral.gt(zeroBig) && bitcoinPrice
        ? collateral.times(bitcoinPrice)
        : zeroBig;

    const netValue =
      collateral.gt(zeroBig) && debt.gt(zeroBig) && bitcoinPrice
        ? collateral.times(bitcoinPrice).minus(debt.times(usduPrice))
        : zeroBig;

    const liquidationPrice =
      collateral.gt(zeroBig) && debt.gt(zeroBig)
        ? debt.times(minCollateralizationRatio).div(collateral)
        : zeroBig;
    ...
    const collateralRatio =
      debt.gt(zeroBig) && collateral.gt(zeroBig) && bitcoinPrice
